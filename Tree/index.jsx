 ...
 * @requires `prop-types`
 *
 * @description Renders content inside an ordered list using the role 'tree'
 *
 * @example
...
class Tree extends Component {
  constructor(props) {
    ...
    React.Children.forEach(props.children, (child, index) => {
      ...
      if (typeof type !== 'function' || (type.name !== 'TreeItem' && type.name !== 'TreeGroup')) {
    ...
    // a Tree is expandable if there is an 'expanded' property _or_ if there is more than
    // one child
    this.expandable = typeof props.expanded !== 'undefined' || items > 1;

    // a Tree can be either expanded or collapsed
    const expanded = props.expanded === 'true' || props.expanded === true || items > 1;
  ...
  /**
   * @property expanded
   * @description Whether or not the tree is expanded so all items are visible
   * @type {boolean}
   */
  get expanded() {
  ...
  onKeyDown = (e) => {
    ...
    e.stopPropagation();
    if (this.shortcuts[key] && this.shortcuts[key].call) {
      this.shortcuts[key]();
    }
  };
  ...
  /**
   * @method stepInto
   * @description Moves into the tree
   * @returns {undefined}
   */
  stepInto = () => {
    this.moveTo(this.element.firstChild);
